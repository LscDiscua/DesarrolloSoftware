...
namespace Sistema_de_Inventario
{
    class ClaCompra
    {
        ...
        {
            if (conexion.IUD(string.Format("INSERT INTO encabezadoCompra (numeroFactura, proveedor, fecha, facturaProveedor ) value('{0}','{1}','{2}','{3}')", NumeroFactura, Proveedor
        ...
        {
            if (conexion.IUD(string.Format("INSERT INTO detalleCompra (idDetalleCompra, encabezadoCompra, producto, cantidad, precio, impuesto, subtotal, total ) " +
                "value('{0}','{1}','{2}', '{3}','{4}', '{5}', '{6}', '{7}')", IdDetalleCompra, EncabezadoCompra, Producto, Cantidad,Precio, Impuesto, SubTotal,Total)))
            {
                return true;
            }
            else
        ...
        {
            DataTable t1 = conexion.consulta(string.Format("SELECT numeroFactura, proveedor, fecha, facturaProveedor FROM taller.encabezadoCompra where numeroFactura='{0}'", no));
            if (t1.Rows.Count > 0)
            {
        ...
        {
            DataTable t1 = conexion.consulta(string.Format("SELECT idDetalleCompra, encabezadoCompra, producto, cantidad, precio, impuesto, subtotal, total FROM taller.detalleCompra where idDetalleCompra ={0}", no));
            if (t1.Rows.Count > 0)
            {
