...
namespace Sistema_de_Inventario.Clases
{
    class ClaProducto
    {
        ...
        {
            if (conexion.IUD(string.Format("UPDATE producto SET nombre='{0}', categoria={1}, marca ={2}, año, = '{3}', proveedor = {4}  " +
                "WHERE idProducto={2}", Nombre, Categoria, Marca, Año, Proveedor, IdProducto)))
            {
                return true;
            }
            else
